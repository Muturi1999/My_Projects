...
//     //   uberWebhookService: uberWebhookService,
//     //   logger: logger,
//     //   environment: 'dev',   //running in dev enviroment
...
//     port: EnvConfig.port,
//     uberEatsConfig: UberEatsConfig(
//       clientId: EnvConfig.uberClientId,
//       clientSecret: EnvConfig.uberClientSecret,
//       environment: EnvConfig.uberEnvironment,
//     ),
//   );
// }
...
class PosApiServer {
  ...
  Future<void> _initializeDependencies() async {
    ...
    webhookController = WebhookController(
      ...
      logger: logger,
      environment: config.uberEatsConfig.environment,
      skipSignatureVerification: EnvConfig.skipWebhookSignatureVerification,
  ...
  Future<void> start() async {
    try {
      ...
      logger.info('  ├─ Uber Eats: ${_getWebhookUrl('uber-eats')} ✅');
      logger.info('  ├─ Glovo:     ${_getWebhookUrl('glovo')} (not implemented)');
      logger.info('  └─ Bolt:      ${_getWebhookUrl('bolt')} (not implemented)');
      logger.info('🔍 Management Endpoints:');
      logger.info('  ├─ Health:    ${_getHealthCheckUrl()}');
      logger.info('  ├─ Metrics:   ${_getMetricsUrl()} (requires auth)');
      logger.info('  └─ Webhooks:  ${_getAllWebhooksUrl()}');
    } catch (e) {
      logger.error('Failed to start server: $e', error: e);
      rethrow;
    }
  ...
  // ✅ Updated helper methods with specific platform support
  String _getWebhookUrl(String platform) {
    final protocol = config.sslConfig != null ? 'https' : 'http';
    return '$protocol://${config.host}:${config.port}/webhooks/$platform';
  }
  ...
  String _getMetricsUrl() {
    final protocol = config.sslConfig != null ? 'https' : 'http';
    return '$protocol://${config.host}:${config.port}/metrics';
  }

  String _getAllWebhooksUrl() {
    final protocol = config.sslConfig != null ? 'https' : 'http';
    return '$protocol://${config.host}:${config.port}/webhooks';
  }
}
